...
let TimetableService = class TimetableService {
    ...
    getTenantId() {
        ...
        if (!tenantId) {
            throw new common_1.BadRequestException('No active school tenant context found. Ensure X-Tenant-ID header or subdomain is provided.');
        }
        return tenantId;
    }
    async getMySchedule(userId, tenantId) {
    ...
    async createClass(name, academicYearId) {
        ...
        if (!targetYearId) {
            ...
            if (!activeYear) {
                throw new common_1.NotFoundException('Please create an Academic Year first.');
            }
    ...
    async getSubjectsForClassSection(classSectionId) {
        const tenantId = this.getTenantId();
    ...
    async createTeacherWithSkills(dto) {
        const tenantId = this.getTenantId();
    ...
    async bulkCreateTeachers(teachers) {
        const tenantId = this.getTenantId();
        const created = [];
        const skipped = [];
        const passwordHash = await bcrypt.hash('StaffPass@123', 10);
    ...
    async getTeachersForSubject(subjectIds) {
        const tenantId = this.getTenantId();
    ...
    async getTeachersForSubjectInClass(subjectId, classSectionId) {
        const tenantId = this.getTenantId();
        const skills = await this.prisma.teacherSkill.findMany({
    ...
    async savePeriodTimings(dto) {
        const tenantId = this.getTenantId();
        await this.prisma.periodTiming.deleteMany({ where: { tenantId } });
        const created = [];
        for (const pt of dto) {
            created.push(await this.prisma.periodTiming.create({
                data: {
    ...
    async getTimetableForClass(classSectionId, academicYearId, startDate, endDate) {
        const tenantId = this.getTenantId();
    ...
    async saveTimetablePeriods(dto) {
        const tenantId = this.getTenantId();
    ...
    async saveSubstituteForPeriod(periodId, substituteTeacherId) {
        const tenantId = this.getTenantId();
    ...
    async getWorkloadSummary(academicYearId) {
        const tenantId = this.getTenantId();
    ...
    async getClassSectionWorkload(id) {
        const tenantId = this.getTenantId();
    ...
    async updateTeacherAssignment(id, newTeacherId, periodsPerWeek) {
        const tenantId = this.getTenantId();
    ...
    async getPeriodsForTeacher(teacherId) {
        const tenantId = this.getTenantId();
        const periods = await this.prisma.period.findMany({
    ...
    async getPeriodsForTeacherWithGaps(teacherId) {
        return this.getPeriodsForTeacher(teacherId);
    }
    async getLeaserPeriodsForTeacher(teacherId) {
        const tenantId = this.getTenantId();
        const periods = await this.prisma.period.findMany({
    ...
    async saveTimetableConfig(dto) {
        ...
        for (const pt of dto.periods) {
            createdPeriods.push(await this.prisma.periodTiming.create({
                data: {
                    periodNumber: pt.periodNumber,
                    name: pt.name,
                    isBreak: pt.isBreak ?? false,
