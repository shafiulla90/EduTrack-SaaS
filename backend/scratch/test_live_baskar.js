...
async function run() {
  ...
  // Query classes
  try {
    const classesRes = await axios.get('https://edu-track-saas-orcin.vercel.app/_backend/teacher-portal/attendance/classes', {
      headers: {
    ...
    console.error("Classes query failed:", err.response ? err.response.data : err.message);
  }

  // Query sections for Class-2
  try {
    const sectionsRes = await axios.get('https://edu-track-saas-orcin.vercel.app/_backend/teacher-portal/attendance/sections?classVal=Class-2', {
      headers: {
        'Authorization': `Bearer ${token}`,
        'X-Tenant-ID': tenantId
      }
    ...
    console.error("Sections query failed:", err.response ? err.response.data : err.message);
  }
}
