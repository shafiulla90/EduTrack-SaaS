    ...
    <div className="space-y-6 max-w-4xl mx-auto animate-fade-in relative">
      <div>
        <h2 className="text-2xl font-black text-slate-100 flex items-center gap-2">
          Apply Student Leave: <span className="text-brand-300 font-extrabold">{selectedChild.name}</span>
        </h2>
        <p className="text-slate-400 text-xs mt-1 font-light">Submit absenteeism notices for medical recovery or emergency situations.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-900/40 border border-slate-850 p-6 rounded-3xl shadow-xl">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Leave Type</label>
                <select
                  value={leaveType}
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Description / Reason</label>
              <textarea
                value={reason}
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Attachment (Optional, e.g. Doctor's Note)</label>
              <div className="border border-dashed border-slate-800 bg-slate-950/30 hover:border-slate-700 rounded-2xl p-4 text-center cursor-pointer transition-all relative flex items-center justify-center gap-2">
                <input
                  type="file"
                ...
                <span className="text-xs text-slate-400 font-semibold block truncate max-w-[200px]">
        ...
        <div className="space-y-4">
          <h3 className="font-bold text-sm text-slate-500 uppercase tracking-widest">Application History</h3>
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-center text-[10px]">
                        <span className="font-bold text-brand-400">{leave.leaveType} Leave</span>
                      ...
                      <div>
                        <span className="text-[9px] text-slate-500 font-medium block">Dates</span>
                        <strong className="text-xs text-slate-300 font-semibold">{leave.startDate} to {leave.endDate}</strong>
                      </div>
                      <p className="text-[11px] text-slate-400 font-light truncate">{leave.reason}</p>
                      ...
                      {leave.attachmentUrl && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="text-[9px] text-brand-400 hover:underline flex items-center gap-1 font-bold mt-1"
                        >
