    ...
    <div className="space-y-6 max-w-4xl mx-auto animate-fade-in relative">
      <div>
        <h2 className="text-2xl font-black text-slate-100 flex items-center gap-2">
          Billing Ledger: <span className="text-brand-300 font-extrabold">{selectedChild.name}</span>
        </h2>
        <p className="text-slate-400 text-xs mt-1 font-light">Pay outstanding invoices online and check past receipts.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-3.5">
              {invoices.map((inv: any) => {
                ...
                return (
                  <div
                    key={inv.id}
                    className="bg-slate-900/40 border border-slate-850 p-5 rounded-3xl shadow-xl flex flex-col justify-between hover:border-slate-800 transition-all space-y-4"
                  >
                    <div className="flex justify-between items-start gap-4">
                      <div className="min-w-0 flex-1">
                        ...
                        <h4 className="text-sm font-bold text-slate-200 mt-2.5 truncate">{inv.description || 'School Fees Statement'}</h4>
                        <p className="text-[10px] text-slate-500 font-light mt-0.5">Due date: {new Date(inv.dueDate).toLocaleDateString()}</p>
                      </div>

                      <div className="text-right">
                        <span className="text-sm font-black text-slate-100">₹{inv.totalAmount.toLocaleString('en-IN')}</span>
                        {inv.remainingBalance > 0 && (
                          <span className="text-[9px] text-slate-500 block font-light mt-0.5">Balance: ₹{inv.remainingBalance.toLocaleString('en-IN')}</span>
                        )}
                      </div>
                    </div>

                    {/* Breakdown items */}
                    <div className="bg-slate-950/20 border border-slate-900/50 rounded-2xl p-3 text-xs space-y-1.5">
                      {inv.items.map((item: any) => (
                        <div key={item.id} className="flex justify-between text-slate-400 font-light text-[11px]">
                          <span>{item.name}</span>
                    ...
                    {!isPaid && (
                      <div className="border-t border-slate-900 pt-3 flex justify-end">
                        <button
                          ...
                          className="px-4 py-2 rounded-xl bg-brand-500 hover:bg-brand-600 text-white font-bold text-xs transition-all shadow-md flex items-center gap-1.5 cursor-pointer"
                        >
        ...
        <div className="space-y-4">
          <h3 className="font-bold text-sm text-slate-500 uppercase tracking-widest">School Bank & UPI</h3>
          
          <div className="bg-slate-900/40 border border-slate-850 p-6 rounded-3xl shadow-xl space-y-5">
            <div className="space-y-3.5">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Direct Bank Transfer</span>
              <div className="space-y-2.5 text-xs">
                <div>
                  <span className="text-slate-500 block text-[10px]">Bank Name</span>
                  <strong className="text-slate-300">{paymentDetails.bankName || 'Covenant Synergy Bank'}</strong>
                </div>
                <div>
                  <span className="text-slate-500 block text-[10px]">IFSC Code</span>
                  <strong className="text-slate-300">{paymentDetails.bankIFSC || 'COVB0007890'}</strong>
                </div>
                <div>
                  <span className="text-slate-500 block text-[10px]">Account Number</span>
                  <strong className="text-slate-300">{paymentDetails.bankAccountNo || '1234 5678 9012'}</strong>
                </div>
                {paymentDetails.bankBranch && (
                  <div>
                    <span className="text-slate-500 block text-[10px]">Branch Name</span>
                    <strong className="text-slate-300">{paymentDetails.bankBranch}</strong>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="border-t border-slate-900 pt-5 space-y-3.5">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">UPI Merchant Handles</span>
              <div className="space-y-2.5 text-xs">
                <div>
                  <span className="text-slate-500 block text-[10px]">Google Pay ID</span>
                  <strong className="text-slate-300">{paymentDetails.googlePayId || 'gpay@edutrack'}</strong>
                </div>
                <div>
                  <span className="text-slate-500 block text-[10px]">PhonePe ID</span>
                  <strong className="text-slate-300">{paymentDetails.phonePeId || 'phonepe@edutrack'}</strong>
                </div>
              </div>
            </div>
          </div>
      ...
      {activeInvoice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-xs p-4">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl p-6 relative animate-scale-up space-y-5">
            <button
              onClick={() => {
                setActiveInvoice(null);
                setMessage('');
              }}
              className="absolute top-4 right-4 p-1.5 rounded-xl hover:bg-slate-800 text-slate-500 hover:text-slate-300 transition-all cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>

            <div>
              <h3 className="text-base font-bold text-slate-100">Pay Outstanding Fee</h3>
              <p className="text-xs text-slate-500 font-light mt-0.5">{activeInvoice.description}</p>
            </div>
            ...
            <div className="p-4 bg-slate-950/50 border border-slate-850 rounded-2xl flex justify-between items-center text-xs">
              <span className="text-slate-400 font-medium">Amount to Pay</span>
              <strong className="text-lg font-black text-white">₹{activeInvoice.remainingBalance.toLocaleString('en-IN')}</strong>
            </div>
            ...
            <form onSubmit={handleSimulatePayment} className="space-y-4">
              ...
              {paymentMethod !== 'BANK' && (
                <div className="flex flex-col items-center justify-center p-4 bg-slate-950/30 border border-slate-900 rounded-2xl gap-2">
